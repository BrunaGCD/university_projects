###################### 
#exemplo1 
vetorList = [2, 3, 5, 7, 11] 
i = 0 
while i < len(vetorList): 
 print(vetorList[i]) 
 i = i + 1

###################### 
#exemplo2 
number1 = input("digite um numero: ") 
number2 = input("digite outro numero: ") 
if number1 > number2: 
 print("o maior numero eh ---> ", number1)
if number2 > number1: 
 print("o maior numero eh ---> ", number2)
else: 
 print(number1, " = ", number2)
 
###################### 
#exemplo3 
for i in range(50): 
 if i%2 == 0: 
 print (i, 'é par') 
 else: 
 print (i, 'é impar') 

###################### 
# teste 4: Tuplas, booleanos e operadores lógicos
tupla_vazia = ()
tupla_cheia = (1, True, False, "texto")
teste_logico = (True and False) or not True

###################### 
# teste 5: Operadores aritméticos
a = 10 ** 2
b = a % 3
c = ~a + (5 * 2) / 2 - 1

###################### 
# teste 6: Operadores extras (is, in, <>)
if a <> b:
    print("a eh diferente de b")

if c is a:
    print("c eh a")

x = a in tupla_cheia